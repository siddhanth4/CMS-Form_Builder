...
// src/Organization/addOrganisation.tsx
// ✅ Only change from original: OrgCode is now shown prominently in the table
//    (with a copy button) so the service provider can share it with the company admin.
//    Everything else — UI, modals, validation, API calls — is unchanged.
...
const initialAddOrg: AddOrgForm = {
    OrgCode: "", OwnerName: "", MobileNo: "", OrgName: "",
    FullName: "", AdminUsername: "", AdminPassword: "",
    PinCode: "", State: "", City: "", Area: "",
    Address: "", Status: "", LogoImg: null,
};

type AddOrgTouched = Record<keyof AddOrgForm, boolean>;

const initialTouched: AddOrgTouched = {
    OrgCode: false, OwnerName: false, MobileNo: false, OrgName: false,
    FullName: false, AdminUsername: false, AdminPassword: false,
    PinCode: false, State: false, City: false, Area: false,
    Address: false, Status: false, LogoImg: false,
};
...
// ✅ Small helper — copy OrgCode to clipboard with visual feedback
const CopyOrgCode: React.FC<{ code: number | string }> = ({ code }) => {
    ...
    return (
        <button
            type="button"
            onClick={handleCopy}
            title="Copy OrgCode — share this with the company admin for their login"
            style={{
                background: "none",
                border: "none",
                padding: "0 4px",
                cursor: "pointer",
                color: copied ? "#3dd68c" : "#7c9ff7",
                fontSize: 13,
            }}
        >
...
export default function AddOrganization() {
    const {
        organizations, totalOrganizations, orgLoading, params,
        setPage, setPageSize, setSearchString, setStatus, toggleSort, fetchOrganizations,
    } = useOrganizationAuth();
    ...
    const showPopup = (type: "success" | "warning" | "danger", title: string, message: string) => {
        setPopupType(type); setPopupTitle(title); setPopupMessage(message); setPopupOpen(true);
    };

    const askConfirm = (message: string, onYes: () => Promise<void>) => {
        pendingSubmitRef.current = onYes;
        setPopupType("warning"); setPopupTitle("Confirm"); setPopupMessage(message);
        setConfirmOpen(true);
    };
    ...
    const totalPages = useMemo(() =>
        Math.max(1, Math.ceil((totalOrganizations || 0) / params.pageSize)),
    [totalOrganizations, params.pageSize]);
    ...
    const COL = {
        OrgName: "OrgName", OwnerName: "OwnerName", MobileNo: "MobileNo",
        City: "City", Status: "Status", TTime: "TTime",
    } as const;
    ...
    useEffect(() => {
        ...
        if (!el) return;
        const onHidden = () => {
            setAddTriedSubmit(false); setTouched(initialTouched);
            setAddUser(initialAddOrg); setLogoPreview("");
        };
        el.addEventListener("hidden.bs.modal", onHidden);
        return () => el.removeEventListener("hidden.bs.modal", onHidden);
    }, []);
    ...
    const openAddUserModal = () => {
        ...
        setAddTriedSubmit(false); setTouched(initialTouched);
        setAddUser({ ...initialAddOrg, OrgCode: "0", Status: "Y" }); setLogoPreview("");
        requestAnimationFrame(() => addModalInstanceRef.current?.show());
    };

    const closeAddUserModal = () => addModalInstanceRef.current?.hide();

    const markTouched       = (field: keyof AddOrgForm) => setTouched((prev) => ({ ...prev, [field]: true }));
    const shouldShowError   = (field: keyof AddOrgForm) => addTriedSubmit || touched[field];

    /* ================= VALIDATION ================= */
    const addOrgErrors = useMemo(() => {
        const e: Record<keyof AddOrgForm, string> = {
            OrgCode: "", OwnerName: "", MobileNo: "", OrgName: "", FullName: "",
            AdminUsername: "", AdminPassword: "", PinCode: "", State: "", City: "",
            Area: "", Address: "", Status: "", LogoImg: "",
        };
    ...
    const addOrgHasError  = useMemo(() => Object.values(addOrgErrors).some(Boolean), [addOrgErrors]);
    const isSaveDisabled  = useMemo(() => addOrgHasError || confirmLoading, [addOrgHasError, confirmLoading]);

    const onAddOrgSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setAddTriedSubmit(true);
        setTouched({
            OrgCode: true, OwnerName: true, MobileNo: true, OrgName: true, FullName: true,
            AdminUsername: true, AdminPassword: true, PinCode: true, State: true,
            City: true, Area: true, Address: true, Status: true, LogoImg: true,
        });
        if (addOrgHasError) return;
    ...
    const onAddOrgReset = () => {
        setAddTriedSubmit(false); setTouched(initialTouched);
        setAddUser(mode === "create"
            ? { ...initialAddOrg, OrgCode: "0", Status: "Y" }
            : { ...initialAddOrg, OrgCode: addUser.OrgCode, Status: addUser.Status || "Y" }
        );
        setLogoPreview("");
    };

    const openEditModal = (row: any) => {
        setMode("edit"); setEditingId(row.Id ?? null);
        setAddTriedSubmit(false); setTouched(initialTouched);
        setAddUser({
            OrgCode: String(row.OrgCode ?? "0"), OwnerName: row.OwnerName ?? "",
            MobileNo: row.MobileNo ?? "", OrgName: row.OrgName ?? "",
            FullName: row.FullName ?? "", AdminUsername: row.AdminUsername ?? "",
            AdminPassword: "", PinCode: row.PinCode ?? "", State: row.State ?? "",
            City: row.City ?? "", Area: row.Area ?? "", Address: row.Address ?? "",
            Status: row.Status ?? "Y", LogoImg: null,
        });
        setLogoPreview(row.LogoImgUrl ?? "");
        requestAnimationFrame(() => addModalInstanceRef.current?.show());
    };
    ...
    return (
        <>
                ...
                <div className="row g-0">
                    ...
                    <div className="panel mb-3">
                        ...
                        <div className="p-3 d-flex flex-wrap gap-2 align-items-center justify-content-between">
                            ...
                            <div className="d-flex gap-2 align-items-center">
                                <select
                                    className="form-select search" style={{ maxWidth: 170 }}
                                    value={params.status}
                                    onChange={(e) => setStatus(e.target.value as "Y" | "N")}
                                >
                                    <option value="Y">Active (Y)</option>
                        ...
                        <div className="p-3">
                            <div className="table-responsive">
                                <table className="table align-middle mb-0">
                                    <thead>
                                        <tr>
                                            <th role="button" style={{ minWidth: 260 }} onClick={() => toggleSort(COL.OrgName)} title="Sort by OrgName">
                                                Organization
                                                <SortIcon active={params.sortColumn === COL.OrgName} order={params.sortOrder} />
                                            </th>

                                            {/* ✅ OrgCode column — prominently shown so SP can share it with admin */}
                                            <th style={{ minWidth: 130 }} title="Share this OrgCode with the company admin for their Admin Portal login">
                                                Org Code
                                                <i className="bi bi-info-circle ms-1 text-secondary" style={{ fontSize: 11 }} title="Admin must enter this code when logging into the Admin Portal" />
                                            </th>
                                    ...
                                    <tbody>
                                        {orgLoading && Array.from({ length: params.pageSize }).map((_, i) => (
                                            <tr key={`sk-${i}`}>
                                                <td>
                                                    <div className="d-flex align-items-center gap-3">
                                                        <div style={{ width: 42, height: 42, borderRadius: 12, overflow: "hidden" }}>
                                                ...
                                                <td><Skeleton width={60} height={24} radius={6} /></td>
                                                <td><Skeleton width="70%" height={14} /></td>
                                                <td><Skeleton width="60%" height={14} /></td>
                                                <td><Skeleton width="60%" height={14} /></td>
                                                <td><Skeleton width={70} height={24} radius={999} /></td>
                                            ...
                                            <tr key={o.Id}>
                                                <td>
                                                    <div className="d-flex align-items-center gap-3">
                                                        ...
                                                        <div>
                                                            <div className="fw-semibold">{o.OrgName}</div>
                                                            <div className="text-secondary small">{o.TType}</div>
                                                        </div>
                                                    </div>
                                                </td>

                                                {/* ✅ OrgCode cell — clearly displayed with copy button */}
                                                <td>
                                                ...
                                                <td className="text-end">
                                                    <button className="btn btn-outline-secondary btn-sm me-2" type="button" onClick={() => openEditModal(o)}>
                                                        <i className="bi bi-pencil" /> Edit
                                                    </button>
                                                    <button className="btn btn-outline-danger btn-sm" type="button">
                                                        <i className="bi bi-trash" /> Delete
                            ...
                            <div className="d-flex flex-wrap gap-2 align-items-center justify-content-between mt-3">
                                ...
                                <div className="d-flex gap-1 flex-wrap">
                                    <button className="btn btn-outline-secondary btn-sm" disabled={!canPrev || orgLoading} onClick={() => setPage(params.page - 1)}>
                                        <i className="bi bi-chevron-left" /> Prev
                                    </button>

                                    {pageNumbers[0] > 1 && (
                                        <>
                                            <button className="btn btn-outline-secondary btn-sm" disabled={orgLoading} onClick={() => setPage(1)}>1</button>
                                            {pageNumbers[0] > 2 && <span className="px-2 text-secondary">...</span>}
                                        </>
                                    )}
                                    ...
                                    <button className="btn btn-outline-secondary btn-sm" disabled={!canNext || orgLoading} onClick={() => setPage(params.page + 1)}>
                                        Next <i className="bi bi-chevron-right" />
                                    </button>
                                </div>
                            </div>
            ...
            {/* ── ADD / EDIT ORGANIZATION MODAL (unchanged) ───────── */}
            <div ref={addModalElRef} className="modal fade" id="addUserModal" tabIndex={-1} aria-hidden="true">
                <div className="modal-dialog modal-lg modal-dialog-scrollable">
                    <div
                        className="modal-content"
                    ...
                    >
                        ...
                        <div className="modal-body p-0">
                            ...
                            <div className="p-3 p-md-4">
                                <form id="addUserForm" noValidate onSubmit={onAddOrgSubmit} onReset={onAddOrgReset}>
                                    <div className="row g-3">
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <input
                                                className={`form-control ${shouldShowError("OwnerName") && addOrgErrors.OwnerName ? "is-invalid" : ""}`}
                                                value={addUser.OwnerName}
                                                onChange={(e) => { markTouched("OwnerName"); setAddUser(p => ({ ...p, OwnerName: e.target.value })); }}
                                                onBlur={() => markTouched("OwnerName")}
                                                placeholder="Owner name"
                                            />
                                            <div className="invalid-feedback">{addOrgErrors.OwnerName}</div>
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <input
                                                ...
                                                className={`form-control ${shouldShowError("MobileNo") && addOrgErrors.MobileNo ? "is-invalid" : ""}`}
                                                value={addUser.MobileNo}
                                                onChange={(e) => { markTouched("MobileNo"); const value = e.target.value.replace(/\D/g, "").slice(0, 10); setAddUser(p => ({ ...p, MobileNo: value })); }}
                                                onBlur={() => markTouched("MobileNo")}
                                                onKeyDown={(e) => { if (!/[0-9]/.test(e.key) && !["Backspace", "Tab", "ArrowLeft", "ArrowRight", "Delete"].includes(e.key)) e.preventDefault(); }}
                                                placeholder="10 digit mobile"
                                            />
                                            <div className="invalid-feedback">{addOrgErrors.MobileNo}</div>
                                        </div>
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <input
                                                className={`form-control ${shouldShowError("OrgName") && addOrgErrors.OrgName ? "is-invalid" : ""}`}
                                                value={addUser.OrgName}
                                                onChange={(e) => { markTouched("OrgName"); setAddUser(p => ({ ...p, OrgName: e.target.value })); }}
                                                onBlur={() => markTouched("OrgName")}
                                                placeholder="Organization name"
                                            />
                                            <div className="invalid-feedback">{addOrgErrors.OrgName}</div>
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <input
                                                className={`form-control ${shouldShowError("FullName") && addOrgErrors.FullName ? "is-invalid" : ""}`}
                                                value={addUser.FullName}
                                                onChange={(e) => { markTouched("FullName"); setAddUser(p => ({ ...p, FullName: e.target.value })); }}
                                                onBlur={() => markTouched("FullName")}
                                                placeholder="Full name"
                                            />
                                            <div className="invalid-feedback">{addOrgErrors.FullName}</div>
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <input
                                                className={`form-control ${shouldShowError("AdminUsername") && addOrgErrors.AdminUsername ? "is-invalid" : ""}`}
                                                value={addUser.AdminUsername}
                                                onChange={(e) => { markTouched("AdminUsername"); setAddUser(p => ({ ...p, AdminUsername: e.target.value })); }}
                                                onBlur={() => markTouched("AdminUsername")}
                                                placeholder="Admin username"
                                            />
                                            <div className="invalid-feedback">{addOrgErrors.AdminUsername}</div>
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <input
                                                ...
                                                className={`form-control ${shouldShowError("AdminPassword") && addOrgErrors.AdminPassword ? "is-invalid" : ""}`}
                                                value={addUser.AdminPassword}
                                                onChange={(e) => { markTouched("AdminPassword"); setAddUser(p => ({ ...p, AdminPassword: e.target.value })); }}
                                                onBlur={() => markTouched("AdminPassword")}
                                                placeholder="Min 6 chars"
                                            />
                                            <div className="invalid-feedback">{addOrgErrors.AdminPassword}</div>
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <input
                                                ...
                                                className={`form-control ${shouldShowError("PinCode") && addOrgErrors.PinCode ? "is-invalid" : ""}`}
                                                value={addUser.PinCode}
                                                onChange={(e) => { markTouched("PinCode"); const value = e.target.value.replace(/\D/g, "").slice(0, 6); setAddUser(p => ({ ...p, PinCode: value })); }}
                                                onBlur={() => markTouched("PinCode")}
                                                onKeyDown={(e) => { if (!/[0-9]/.test(e.key) && !["Backspace", "Tab", "ArrowLeft", "ArrowRight", "Delete"].includes(e.key)) e.preventDefault(); }}
                                                placeholder="440034"
                                            />
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <input
                                                className={`form-control ${shouldShowError("State") && addOrgErrors.State ? "is-invalid" : ""}`}
                                                value={addUser.State}
                                                onChange={(e) => { markTouched("State"); setAddUser(p => ({ ...p, State: e.target.value })); }}
                                                onBlur={() => markTouched("State")}
                                                placeholder="Maharashtra"
                                            />
                                            <div className="invalid-feedback">{addOrgErrors.State}</div>
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <input
                                                className={`form-control ${shouldShowError("City") && addOrgErrors.City ? "is-invalid" : ""}`}
                                                value={addUser.City}
                                                onChange={(e) => { markTouched("City"); setAddUser(p => ({ ...p, City: e.target.value })); }}
                                                onBlur={() => markTouched("City")}
                                                placeholder="Nagpur"
                                            />
                                            <div className="invalid-feedback">{addOrgErrors.City}</div>
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <input
                                                className={`form-control ${shouldShowError("Area") && addOrgErrors.Area ? "is-invalid" : ""}`}
                                                value={addUser.Area}
                                                onChange={(e) => { markTouched("Area"); setAddUser(p => ({ ...p, Area: e.target.value })); }}
                                                onBlur={() => markTouched("Area")}
                                                placeholder="Hudkeshwar"
                                            />
                                            <div className="invalid-feedback">{addOrgErrors.Area}</div>
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <input
                                                className={`form-control ${shouldShowError("Address") && addOrgErrors.Address ? "is-invalid" : ""}`}
                                                value={addUser.Address}
                                                onChange={(e) => { markTouched("Address"); setAddUser(p => ({ ...p, Address: e.target.value })); }}
                                                onBlur={() => markTouched("Address")}
                                                placeholder="Full address"
                                            />
                                            <div className="invalid-feedback">{addOrgErrors.Address}</div>
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <select
                                                className={`form-select ${shouldShowError("Status") && addOrgErrors.Status ? "is-invalid" : ""}`}
                                                value={addUser.Status}
                                                onChange={(e) => { markTouched("Status"); setAddUser(p => ({ ...p, Status: e.target.value })); }}
                                                onBlur={() => markTouched("Status")}
                                            >
                                                <option value="Y">Active (Y)</option>
                                                <option value="N">Inactive (N)</option>
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    markTouched("LogoImg");
                                                    const f = e.target.files?.[0] || null;
                                                    if (!f) { setAddUser(p => ({ ...p, LogoImg: null })); setLogoPreview(""); return; }
                                                    if (f.size > 2 * 1024 * 1024) {
                                                        showPopup("warning", "Invalid File", "Logo image must be less than 2MB");
                                                        e.target.value = ""; setAddUser(p => ({ ...p, LogoImg: null })); setLogoPreview(""); return;
                                                    }
                                                    setAddUser(p => ({ ...p, LogoImg: f }));
                                                    setLogoPreview(URL.createObjectURL(f));
                                                }}
                                        ...
                                        <div className="col-12 col-md-6">
                                            ...
                                            <div
                                                className="d-flex align-items-center justify-content-center h-100 rounded"
                                                style={{ minHeight: 120, border: "1px dashed #ced4da", background: "#f8f9fa" }}
                                            >
                                                {logoPreview ? (
                                                    <img src={logoPreview} alt="preview" style={{ maxHeight: 100, maxWidth: "100%", objectFit: "contain", borderRadius: 10 }} />
                                                ) : (
                                                    <span className="text-secondary small">No image selected</span>
                                                )}
                                            </div>
                                        ...
                                        <div className="col-12 d-flex flex-wrap gap-2 justify-content-end pt-2">
                                            <button type="reset" className="btn btn-outline-secondary" style={{ borderRadius: 14 }}>Reset</button>
                                            <button className="btn btn-outline-secondary" type="button" onClick={closeAddUserModal} style={{ borderRadius: 14 }}>Close</button>
                                            <button type="submit" className="btn btn-brand px-4" disabled={isSaveDisabled}>
                                                {confirmLoading
                                                    ? "Saving..."
                                                    : mode === "create" ? "Save Organization" : "Update Organization"}{" "}
                                                <i className="bi bi-check2 ms-2" />
                                            </button>
                                        </div>
            ...
            <PopupAlert
                ...
                confirmText={confirmLoading ? "Saving..." : "Yes"}
                cancelText="No"
                onConfirm={async () => { if (confirmLoading) return; await pendingSubmitRef.current?.(); }}
                onCancel={() => closeConfirm()}
            />
