...
type AdminForm = {
    AdCode: number; FullName: string; MobileNo: string; EmailId: string;
    AdminUsername: string; AdminPassword: string; Type: string;
    RoleId: number; Status: "Y" | "N"; Address: string;
};

const initialAdminForm: AdminForm = {
    AdCode: 0, FullName: "", MobileNo: "", EmailId: "", AdminUsername: "", AdminPassword: "",
    Type: "", RoleId: 0, Status: "Y", Address: "",
};
...
/* 🔥 STRICT MAPPER: Forces the backend names to match exactly what you want */
const getExactLabel = (m: any) => {
    const route = (m.Route || "").toLowerCase();
    const name = (m.PageName || "").toLowerCase();
    const key = (m.PageKey || "").toLowerCase();

    if (route.includes("dashboard") || name.includes("dashboard") || key.includes("dashboard")) return { label: "Dashboard", route: "/admin/dashboard", icon: "bi-speedometer2" };
    if (route.includes("withdraw") || name.includes("withdraw") || key.includes("withdraw")) return { label: "Withdraw Requests", route: "/admin/withdrawRequest", icon: "bi-shield-x" };
    if (route.includes("grievance") || name.includes("grievance") || key.includes("grievance")) return { label: "Grievances", route: "/admin/grievances", icon: "bi-exclamation-octagon" };
    if (route.includes("privacy") || name.includes("privacy") || key.includes("privacy")) return { label: "Privacy Notices", route: "/admin/privacyNotices", icon: "bi-file-earmark-lock2" };
    if (route.includes("builder") || name.includes("new form") || key.includes("builder")) return { label: "Add New Form", route: "/admin/builder", icon: "bi-ui-checks-grid" };
    if (route.includes("submission") || name.includes("response") || key.includes("response")) return { label: "Form Responses", route: "/admin/submissions", icon: "bi-envelope-paper" };
    if (route.includes("forms") || name.includes("form list") || key.includes("formlist")) return { label: "Form List", route: "/admin/forms", icon: "bi-list-check" };
    if (route.includes("addadmin") || name.includes("user") || name.includes("role") || key.includes("user")) return { label: "Role List", route: "/admin/addAdmin", icon: "bi-people" };

    return { label: m.PageName || "Unknown", route: m.Route, icon: m.Icon || "bi-circle" };
};

export default function AddAdmin() {
    const {
        adminList, adminsTotal, adminsLoading, adminsError, refreshAdmins, adminParams,
        menus, menusLoading, menusError, refreshMenus, roles, rolesLoading, rolesError, refreshRoles,
    } = useProject();
    ...
    const totalPages = useMemo(() => Math.max(1, Math.ceil((adminsTotal || 0) / pageSize)), [adminsTotal, pageSize]);
    const goToPage = (p: number) => {
        const next = Math.max(1, Math.min(totalPages, p));
        refreshAdmins({ PageNumber: next, PageSize: pageSize, status: adminParams.status });
    };

    /* 🔥 CORE BUILDER: Builds the exact array in the exact order requested */
    const buildPermissionsState = (backendMenus: any[], assignedPageIds: Set<number>, targetAdCode: number) => {
        let perms = backendMenus.map((m) => {
            const exact = getExactLabel(m);
            return {
                pageId: m.Id,
                key: m.PageKey || exact.label,
                label: exact.label,
                enabled: assignedPageIds.has(Number(m.Id)),
                route: exact.route || m.Route,
                icon: "bi " + exact.icon,
            };
        });

        // Smart Local Engine: Checks if the admin had Privacy/Grievance permissions saved locally
        const hasPrivacyLocal = targetAdCode ? localStorage.getItem(`FRONTEND_PERM_998_${targetAdCode}`) === "Y" : false;
        const hasGrievanceLocal = targetAdCode ? localStorage.getItem(`FRONTEND_PERM_999_${targetAdCode}`) === "Y" : false;

        const existingLabels = new Set(perms.map(p => p.label));
        if (!existingLabels.has("Privacy Notices")) {
            perms.push({ pageId: 998, key: "privacy", label: "Privacy Notices", enabled: assignedPageIds.has(998) || hasPrivacyLocal, icon: "bi bi-file-earmark-lock2", route: "/admin/privacyNotices" });
        }
        if (!existingLabels.has("Grievances")) {
            perms.push({ pageId: 999, key: "grievances", label: "Grievances", enabled: assignedPageIds.has(999) || hasGrievanceLocal, icon: "bi bi-exclamation-octagon", route: "/admin/grievances" });
        }

        perms.sort((a, b) => {
            let indexA = EXACT_ORDER.indexOf(a.label);
            let indexB = EXACT_ORDER.indexOf(b.label);
    ...
    const loadPermissionsForAdmin = async (adCode: number) => {
        ...
        const assignedPageIds = new Set(assigned.map((x) => Number(x.PageId)));
        const activeMenus = menus.filter((m) => m.Status === "Y");
        setPermissions(buildPermissionsState(activeMenus, assignedPageIds, adCode));
    };

    const buildPermissionsByRole = () => {
        const activeMenus = menus.filter((m) => m.Status === "Y");
        return buildPermissionsState(activeMenus, new Set(), 0); 
    };

    const openCreateAdminModal = () => {
        setSaveError(""); setAddTriedSubmit(false);
        const firstRole = roles.find((r) => r.Status === "Y");
        setAdminForm({ ...initialAdminForm, RoleId: firstRole?.Id ?? 0, Type: firstRole?.Role ?? "" });
        setPermissions(buildPermissionsByRole());
        setActiveTab("details");
        requestAnimationFrame(() => {
            addModalInstanceRef.current?.show();
            setActiveTab("details");
    ...
    const openUpdateAdminModal = async (row: any) => {
        setSaveError(""); setAddTriedSubmit(false); setActiveTab("details");
        const matchedRoleById = roles.find((r) => Number(r.Id) === Number(row.RoleId));
        const matchedRoleByName = roles.find((r) => r.Role?.trim().toLowerCase() === String(row.TType ?? "").trim().toLowerCase());
        const matchedRole = matchedRoleById ?? matchedRoleByName;

        setAdminForm({
            AdCode: Number(row.AdCode) || 0, FullName: row.FullName ?? "", MobileNo: row.MobileNo ?? "",
            EmailId: row.EmailId ?? "", AdminUsername: row.Username ?? "", AdminPassword: "",
            Type: matchedRole?.Role ?? row.TType ?? "", RoleId: Number(matchedRole?.Id ?? row.RoleId ?? 0),
            Status: row.Status === "Y" ? "Y" : "N", Address: row.Address ?? "",
        });

        requestAnimationFrame(() => { addModalInstanceRef.current?.show(); setActiveTab("details"); });

        if (menus.length === 0) await refreshMenus({ status: "Y" });
        await loadPermissionsForAdmin(Number(row.AdCode));
    };
    ...
    useEffect(() => {
        if (!menus || menus.length === 0 || !roles.length) return;
        setPermissions((prev) => {
            if (prev.length === 0) return buildPermissionsByRole();
            const prevMap = new Map(prev.map((x) => [x.pageId, x.enabled]));
            const activeMenus = menus.filter((m) => m.Status === "Y");
            return buildPermissionsState(activeMenus, new Set(Array.from(prevMap.entries()).filter(e => e[1]).map(e => e[0])), adminForm.AdCode);
        });
    }, [menus, roles, adminForm.RoleId]);
    ...
    const togglePermission = (pageId: number) => {
        setPermissions((prev) => prev.map((p) => (p.pageId === pageId ? { ...p, enabled: !p.enabled } : p)));
    };

    useEffect(() => {
        if (!modalElRef.current) return;
        modalInstanceRef.current = new Modal(modalElRef.current, { backdrop: true, keyboard: true, focus: true });
        return () => { modalInstanceRef.current?.dispose(); modalInstanceRef.current = null; };
    }, []);

    useEffect(() => {
        if (!addModalElRef.current) return;
        addModalInstanceRef.current = new Modal(addModalElRef.current, { backdrop: "static", keyboard: true, focus: true });
        return () => { addModalInstanceRef.current?.dispose(); addModalInstanceRef.current = null; };
    }, []);
    ...
    useEffect(() => {
        ...
        const onHidden = () => {
            const firstRole = roles.find((r) => r.Status === "Y");
            setSaveError(""); setSaving(false); setAddTriedSubmit(false);
            setAdminForm({ ...initialAdminForm, RoleId: firstRole?.Id ?? 0, Type: firstRole?.Role ?? "" });
            setPermissions(buildPermissionsByRole());
            setActiveTab("details");
        };
    ...
    const filteredAdmins = useMemo(() => {
        ...
        if (s) {
            rows = rows.filter((x) =>
                x.FullName?.toLowerCase().includes(s) || x.Username?.toLowerCase().includes(s) ||
                x.MobileNo?.toLowerCase().includes(s) || (x.EmailId ?? "").toLowerCase().includes(s) ||
                x.TType?.toLowerCase().includes(s)
            );
        }
        rows.sort((a, b) => {
    ...
    const adminErrors = useMemo(() => {
        const e: Partial<Record<keyof AdminForm, string>> = {};
        if (!adminForm.FullName.trim() || adminForm.FullName.trim().length < 2) e.FullName = "Enter full name (min 2 chars).";
        if (!adminForm.MobileNo.trim()) e.MobileNo = "Enter mobile number.";
        else if (!/^\d{10}$/.test(adminForm.MobileNo.trim())) e.MobileNo = "Enter 10 digit mobile number.";
        if (!adminForm.EmailId.trim()) e.EmailId = "Enter email.";
        else if (!isEmailValid(adminForm.EmailId)) e.EmailId = "Enter a valid email.";
        if (!adminForm.AdminUsername.trim()) e.AdminUsername = "Enter username.";
        
        const password = adminForm.AdminPassword.trim();
        if (adminForm.AdCode === 0) {
            if (!password) e.AdminPassword = "Enter password.";
            else if (password.length < 6) e.AdminPassword = "Password must be at least 6 characters.";
        } else {
            if (password && password.length < 6) e.AdminPassword = "Password must be at least 6 characters.";
        }
        if (!adminForm.RoleId) e.RoleId = "Select role.";
        if (!adminForm.Status) e.Status = "Select status.";
        return e;
    }, [adminForm]);

    const adminHasError = useMemo(() => Object.values(adminErrors).some((x) => !!x), [adminErrors]);
    const showEmailError = (addTriedSubmit || adminForm.EmailId.trim().length > 0) && !!adminErrors.EmailId;
    const showPasswordError = (addTriedSubmit || adminForm.AdminPassword.trim().length > 0) && !!adminErrors.AdminPassword;
    const canGoNext = !adminHasError && !saving;
    ...
    const onAddUserSubmit = async (e: React.FormEvent) => {
        ...
        setAddTriedSubmit(true); setSaveError("");
        if (adminHasError) return;
        setPopupMsg(adminForm.AdCode === 0 ? "Are you sure you want to create this admin?" : "Are you sure you want to update this admin?");
        setConfirmOpen(true);
    };
    ...
    const doSaveAdmin = async () => {
        ...
        const payload: AddUpdateAdminPayload = {
            AdCode: adminForm.AdCode, MobileNo: adminForm.MobileNo, FullName: adminForm.FullName,
            AdminUsername: adminForm.AdminUsername, AdminPassword: adminForm.AdminPassword,
            Status: adminForm.Status, Type: role?.Role ?? adminForm.Type, EmailId: adminForm.EmailId,
            Address: adminForm.Address, RoleId: Number(adminForm.RoleId),
            MenusJson: JSON.stringify(menusArray),
        };

        try {
            setSaving(true);
            const res: any = await addUpdateAdmin(payload);
            
            // 🔥 ENGINE LOGIC: If backend deletes the custom PageIds, we store their values locally in the browser!
            let resolvedAdCode = adminForm.AdCode;
            if (resolvedAdCode === 0 && res?.data?.adCode) {
                resolvedAdCode = res.data.adCode;
            }

            if (resolvedAdCode) {
                const pNotice = permissions.find(p => p.pageId === 998)?.enabled ? "Y" : "N";
                const pGrievance = permissions.find(p => p.pageId === 999)?.enabled ? "Y" : "N";
                localStorage.setItem(`FRONTEND_PERM_998_${resolvedAdCode}`, pNotice);
                localStorage.setItem(`FRONTEND_PERM_999_${resolvedAdCode}`, pGrievance);
            }

            await refreshAdmins({ PageNumber: 1, PageSize: 10, status: "Y" });
            closeAddUserModal();
            setPopupMsg(adminForm.AdCode === 0 ? "Admin created successfully." : "Admin updated successfully.");
            setSuccessOpen(true);
        } catch (err: any) {
            setPopupMsg(err?.message || "Failed to save admin.");
            setErrorOpen(true);
    ...
    const goToPermissionsTab = () => {
        if (!canGoNext) { setAddTriedSubmit(true); return; }
        setActiveTab("permissions");
    };

    const goToDetailsTab = () => { setActiveTab("details"); };
        ...
        <>
            <div className="container-fluid app-shell">
                <div className="row g-0">
                    <div className="panel mb-3">
                        <div className="panel-head p-3 d-flex flex-wrap gap-2 align-items-center justify-content-between">
                            <div><div className="h5 mb-1">Roles</div></div>
                            <div className="d-flex gap-2 align-items-center">
                                <button className="btn btn-outline-secondary btn-sm" type="button"><i className="bi bi-download" /> Export CSV</button>
                                <button className="btn btn-outline-secondary btn-sm" type="button" onClick={openCreateAdminModal}><i className="bi bi-person-plus" /> Add Admin Role</button>
                            </div>
                        </div>
                        ...
                        <div className="p-3 d-flex flex-wrap gap-2 align-items-center justify-content-between">
                            <div className="input-group" style={{ maxWidth: 520 }}>
                                ...
                                <input className="form-control search" placeholder="Search..." value={search} onChange={(e) => setSearch(e.target.value)} />
                            </div>

                            <div className="d-flex gap-2">
                                <select className="form-select search" style={{ maxWidth: 190 }} value={range} onChange={(e) => setRange(e.target.value as RangeFilter)}>
                                    <option>All</option><option>Today</option><option>Last 7 days</option><option>Last 30 days</option>
                                </select>
                                <select className="form-select search" style={{ maxWidth: 190 }} value={sort} onChange={(e) => setSort(e.target.value as SortFilter)}>
                                    <option>Newest first</option><option>Oldest first</option>
                                </select>
                                <select className="form-select search" style={{ maxWidth: 140 }} value={pageSize} onChange={(e) => refreshAdmins({ PageNumber: 1, PageSize: Number(e.target.value), status: adminParams.status })}>
                        ...
                        <div className="p-3">
                            <div className="table-responsive">
                                <table className="table align-middle mb-0">
                                    <thead>
                                        <tr>
                                            <th style={{ minWidth: 240 }}>Admin</th><th>Username</th><th>Mobile</th>
                                            <th>Email</th><th>Role</th><th>Status</th><th className="text-end" style={{ minWidth: 240 }}>Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {adminsLoading && <AdminTableSkeleton rows={8} />}
                                        {!adminsLoading && !adminsError && filteredAdmins.map((row) => (
                                            <tr key={row.Id}>
                                                <td>
                                                    <div className="d-flex align-items-center gap-3">
                                                        <div className="avatar">{avatarText(row.FullName || row.Username)}</div>
                                                        <div><div className="fw-semibold">{row.FullName}</div></div>
                                                    </div>
                                                </td>
                                                <td className="text-secondary">{row.Username}</td>
                                                <td className="text-secondary">{row.MobileNo}</td>
                                                ...
                                                <td className="text-secondary">{row.TType}</td>
                                                <td>
                                                    <span className={row.Status === "Y" ? "badge text-bg-success rounded-pill" : "badge text-bg-secondary rounded-pill"}>
                                                        {row.Status === "Y" ? "Active" : "Inactive"}
                                                    </span>
                                                </td>
                                                <td className="text-end">
                                                    <button className="btn btn-outline-secondary btn-sm me-2" type="button" onClick={() => openUpdateAdminModal(row)}>
                                                        <i className="bi bi-pencil" /> Edit
                                                    </button>
                                                    <button className="btn btn-outline-danger btn-sm" type="button" onClick={() => console.log("Delete Admin", row.Id)}>
                                                        <i className="bi bi-trash" /> Delete
                                                    </button>
                                                </td>
                                            </tr>
                                        ...
                                        {!adminsLoading && !adminsError && filteredAdmins.length === 0 && (
                                            <tr><td colSpan={7} className="text-center text-secondary py-5">No admins found.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <nav aria-label="Admin pagination">
                                    <ul className="pagination pagination-soft mb-0">
                                        ...
                                        {Array.from({ length: totalPages }, (_, i) => i + 1).slice(Math.max(0, page - 3), Math.min(totalPages, page + 2)).map((p) => (
                                            <li key={p} className={`page-item ${p === page ? "active" : ""}`}>
                                                <button className="page-link" onClick={() => goToPage(p)} type="button" disabled={adminsLoading}>{p}</button>
                                            </li>
                                        ))}
                                        <li className={`page-item ${page >= totalPages || adminsLoading ? "disabled" : ""}`}>
                            ...
                            <div className="text-secondary small mt-2 text-end">Page {page} of {totalPages} • Showing {filteredAdmins.length} records</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div ref={addModalElRef} className="modal fade" id="addUserModal" tabIndex={-1} aria-hidden="true">
                <div className="modal-dialog modal-lg modal-dialog-scrollable">
                    <div className="modal-content" style={{ border: "1px solid var(--stroke)", background: "var(--card)" }}>
                        <div className="modal-header" style={{ borderBottom: "1px solid var(--stroke)" }}>
                            <div className="d-flex align-items-center gap-2">
                                <h5 className="modal-title mb-0">{adminForm.AdCode === 0 ? "Add Role" : "Update User"}</h5>
                            </div>
                            <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close" />
                        </div>
                        ...
                        <div className="modal-body p-0">
                            <div className="px-3 px-md-4 pt-3">
                                <ul className="nav nav-tabs" role="tablist">
                                    <li className="nav-item" role="presentation">
                                        <button className={`nav-link ${activeTab === "details" ? "active" : ""}`} type="button" role="tab" onClick={() => setActiveTab("details")}>
                                            <i className="bi bi-person-lines-fill me-2" />Details
                                        </button>
                                    </li>
                                    <li className="nav-item" role="presentation">
                                        <button className={`nav-link ${activeTab === "permissions" ? "active" : ""}`} type="button" role="tab" onClick={() => { if (!canGoNext) { setAddTriedSubmit(true); return; } setActiveTab("permissions"); }}>
                                            <i className="bi bi-shield-lock me-2" />Permissions
                                        </button>
                                    </li>
                                </ul>
                            </div>
                            ...
                            <div className="tab-content">
                                <div className={`tab-pane fade ${activeTab === "details" ? "show active" : ""}`} role="tabpanel">
                                    <div className="px-3 px-md-4 py-3 d-flex align-items-center justify-content-between">
                                        <div className="fw-bold">Role Details</div>
                                    </div>
                                    <div style={{ borderTop: "1px solid var(--stroke)" }} />
                                    <div className="p-3 p-md-4">
                                        <form id="addUserForm" noValidate onSubmit={onAddUserSubmit} onReset={onAddUserReset}>
                                            <div className="row g-3">
                                                <div className="col-12 col-md-6">
                                                    <label className="form-label fw-semibold">Full Name <span className="req">*</span></label>
                                                    <input className={`form-control ${addTriedSubmit && adminErrors.FullName ? "is-invalid" : ""}`} value={adminForm.FullName} onChange={(e) => setAdminForm((p) => ({ ...p, FullName: e.target.value }))} placeholder="Enter full name" />
                                                    <div className="invalid-feedback">{adminErrors.FullName}</div>
                                                </div>
                                                <div className="col-12 col-md-6">
                                                    <label className="form-label fw-semibold">Mobile <span className="req">*</span></label>
                                                    <input type="text" inputMode="numeric" pattern="[0-9]*" className={`form-control ${addTriedSubmit && adminErrors.MobileNo ? "is-invalid" : ""}`} value={adminForm.MobileNo} onChange={(e) => { const value = e.target.value.replace(/\D/g, "").slice(0, 10); setAdminForm((p) => ({ ...p, MobileNo: value })); }} placeholder="Enter 10 digit mobile" />
                                                    <div className="invalid-feedback">{adminErrors.MobileNo}</div>
                                                </div>
                                                <div className="col-12 col-md-6">
                                                    <label className="form-label fw-semibold">Email <span className="req">*</span></label>
                                                    <input type="email" inputMode="email" autoComplete="email" className={`form-control ${showEmailError ? "is-invalid" : ""}`} value={adminForm.EmailId} onChange={(e) => setAdminForm((p) => ({ ...p, EmailId: e.target.value }))} placeholder="Enter email" />
                                                    <div className="invalid-feedback">{adminErrors.EmailId}</div>
                                                </div>
                                                <div className="col-12 col-md-6">
                                                    <label className="form-label fw-semibold">Role <span className="req">*</span></label>
                                                    <select className={`form-select ${addTriedSubmit && adminErrors.RoleId ? "is-invalid" : ""}`} value={adminForm.RoleId || ""} onChange={(e) => { const roleId = Number(e.target.value); const selectedRole = roles.find((r) => Number(r.Id) === roleId); setAdminForm((p) => ({ ...p, RoleId: roleId, Type: selectedRole?.Role ?? "" })); }} disabled={rolesLoading}>
                                                        <option value="">Select role</option>
                                                        {roles.filter((r) => r.Status === "Y").map((role) => (
                                                ...
                                                <div className="col-12 col-md-6">
                                                    <label className="form-label fw-semibold">Username <span className="req">*</span></label>
                                                    <input className={`form-control ${addTriedSubmit && adminErrors.AdminUsername ? "is-invalid" : ""}`} value={adminForm.AdminUsername} onChange={(e) => setAdminForm((p) => ({ ...p, AdminUsername: e.target.value }))} placeholder="Enter username" />
                                                    <div className="invalid-feedback">{adminErrors.AdminUsername}</div>
                                                </div>
                                                <div className="col-12 col-md-6">
                                                    <label className="form-label fw-semibold">Password {adminForm.AdCode === 0 ? <span className="req">*</span> : <span className="text-secondary small">(leave blank to keep same)</span>}</label>
                                                    <input type="password" inputMode="numeric" className={`form-control ${showPasswordError ? "is-invalid" : ""}`} value={adminForm.AdminPassword} onChange={(e) => setAdminForm((p) => ({ ...p, AdminPassword: e.target.value.replace(/\D/g, "").slice(0, 6) }))} placeholder={adminForm.AdCode === 0 ? "Enter 6 digit password" : "Leave blank to keep same"} />
                                                    <div className="invalid-feedback">{adminErrors.AdminPassword}</div>
                                                </div>
                                                <div className="col-12 col-md-6">
                                                    <label className="form-label fw-semibold">Status <span className="req">*</span></label>
                                                    <select className={`form-select ${addTriedSubmit && adminErrors.Status ? "is-invalid" : ""}`} value={adminForm.Status} onChange={(e) => setAdminForm((p) => ({ ...p, Status: e.target.value as "Y" | "N" }))}>
                                                        <option value="Y">Active</option>
                                                        <option value="N">Inactive</option>
                                                    </select>
                                                    <div className="invalid-feedback">{adminErrors.Status}</div>
                                                ...
                                                <div className="col-12">
                                                    <label className="form-label fw-semibold">Address</label>
                                                    <textarea className="form-control" rows={2} value={adminForm.Address} onChange={(e) => setAdminForm((p) => ({ ...p, Address: e.target.value }))} placeholder="Enter address" />
                                                </div>
                                                {saveError && <div className="col-12"><div className="alert alert-danger mb-0">{saveError}</div></div>}
                                            </div>
                                        </form>
                                    </div>
                                </div>

                                <div className={`tab-pane fade ${activeTab === "permissions" ? "show active" : ""}`} role="tabpanel">
                                    <div className="px-3 px-md-4 py-3 d-flex align-items-center justify-content-between">
                                        <div className="fw-bold">Permissions</div>
                                        <span className="text-secondary small">Check boxes to enable access to specific sections</span>
                                    </div>
                                    <div style={{ borderTop: "1px solid var(--stroke)" }} />
                                    <div className="p-3 p-md-4">
                                        <div className="panel" style={{ border: "1px solid var(--stroke)", borderRadius: 16 }}>
                                            <div className="p-3">
                                                <div className="d-flex text-secondary small fw-semibold pb-2" style={{ borderBottom: "1px solid var(--stroke)" }}>
                                                    <div style={{ flex: 1 }}>Section Name</div>
                                                    <div style={{ width: 140 }} className="text-end">Access</div>
                                                </div>

                                                <div className="pt-2">
                                                    {menusLoading && <div className="text-secondary p-3">Loading permissions list...</div>}
                                                    {!menusLoading && !menusError && permissions.length === 0 && <div className="text-secondary p-3">No pages found.</div>}

                                                    {permissions.map((p) => (
                                                        <div key={p.key} className="d-flex align-items-center py-2" style={{ borderBottom: "1px dashed var(--stroke)" }}>
                                                            <div style={{ flex: 1 }}>
                                                                <div className="fw-semibold d-flex align-items-center gap-2">
                                                ...
                                                <div className="pt-3 d-flex flex-wrap gap-2 justify-content-end">
                                                    <button type="button" className="btn btn-outline-secondary btn-sm" onClick={() => setPermissions((prev) => prev.map((x) => ({ ...x, enabled: true })))} disabled={!canEditPermissions}>Enable All</button>
                                                    <button type="button" className="btn btn-outline-secondary btn-sm" onClick={() => setPermissions((prev) => prev.map((x) => ({ ...x, enabled: false })))} disabled={!canEditPermissions}>Disable All</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="p-3 px-md-4 d-flex flex-wrap gap-2 justify-content-end">
                                {activeTab === "details" ? (
                                    <>
                                        <button type="button" className="btn btn-outline-secondary" onClick={onAddUserReset} style={{ borderRadius: 14 }}>Reset</button>
                                        <button type="button" className="btn btn-outline-secondary" onClick={closeAddUserModal} style={{ borderRadius: 14 }}>Close</button>
                                        <button type="button" className="btn btn-brand px-4" onClick={(e) => { e.preventDefault(); e.stopPropagation(); goToPermissionsTab(); }} disabled={!canGoNext}>Next</button>
                                    </>
                                ) : (
                                    <>
                                        <button type="button" className="btn btn-outline-secondary" onClick={goToDetailsTab} style={{ borderRadius: 14 }}><i className="bi bi-arrow-left me-2" /> Back</button>
                                        <button type="button" className="btn btn-outline-secondary" onClick={closeAddUserModal} style={{ borderRadius: 14 }}>Close</button>
                                        <button type="submit" form="addUserForm" className="btn btn-brand px-4" disabled={saving}>{saving ? "Saving..." : adminForm.AdCode === 0 ? "Create Admin" : "Update Admin"}</button>
                                    </>
                                )}
                            </div>
                        </div>
            ...
            <PopupAlert open={confirmOpen} type="warning" title="Confirm" message={popupMsg} confirmMode={true} confirmText="Yes, Save" cancelText="No" onClose={() => setConfirmOpen(false)} onConfirm={() => { setConfirmOpen(false); doSaveAdmin(); }} onCancel={() => setConfirmOpen(false)} />
            <PopupAlert open={successOpen} type="success" title="Success" message={popupMsg} onClose={() => setSuccessOpen(false)} autoCloseMs={2000} />
            <PopupAlert open={errorOpen} type="danger" title="Error" message={popupMsg} onClose={() => setErrorOpen(false)} />
        </>
    );
}